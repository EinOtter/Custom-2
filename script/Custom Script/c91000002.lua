...
function s.initial_effect(c)
	...
	e5:SetCategory(CATEGORY_TOHAND+CATEGORY_NEGATE+CATEGORY_DESTROY)
	...
	e5:SetCondition(function(e) return not e:GetHandler():IsStatus(STATUS_BATTLE_DESTROYED) and Duel.IsChainNegatable(ev) end)
...
function s.negcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return ep~=tp and re:IsMonsterEffect() and re:IsHasType(EFFECT_TYPE_ACTIVATE) and Duel.IsChainNegatable(ev)
end
...
function s.tgfilter(c)
	return c:IsFaceup() and c:IsSetCard(SET_MEKLORD) and c:IsAbleToHand()
end
...
function s.negop(e,tp,eg,ep,ev,re,r,rp)
	local tc=Duel.GetFirstTarget()
	if tc:IsRelateToEffect(e) and tc:IsFaceup() and tc:IsControler(tp) and tc:IsSetCard(SET_MEKLORD)
