...
function s.cfilter(c,tp)
	return c:IsSetCard(SET_MEKLORD) and c:IsMonster() and not c:IsCode(id) and c:IsAbleToGraveAsCost()
...
function s.spcon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():IsReason(REASON_COST) and re:IsActivated() and re:IsMonsterEffect()
		and re:GetHandler():IsAttribute(RACE_MACHINE)
end
function s.spfilter(c,e,tp)
	return c:IsSetCard(SET_MEKLORD) and not c:IsCode(id) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
